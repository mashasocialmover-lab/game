...
export async function getRoomPlayers(roomId) {
    try {
        ...
        // Ждем немного чтобы PeerJS успел инициализироваться
        if (data && data.length > 1) {
            console.log('🔗 Начинаем подключение к другим игрокам через 1 секунду...');
            setTimeout(async () => {
                try {
                    await connectToAllPlayers(data);
                } catch (error) {
                    console.error('❌ Ошибка подключения к игрокам:', error);
                }
            }, 1000);
        } else {
